...
const { mockImport, reImport, stopAll, enableNestedImports } = createMockImport(
  import.meta.url
);
...
function failOnError(done, err) {
  ...
  expect.fail(
    "error",
    "success",
    "expected success, but got error " + JSON.stringify(err) + err
  );
  done();
}
...
function failOnSuccess(done, user) {
  expect.fail(
    "success",
    "error",
    "expected error, but got success " + JSON.stringify(user)
  );
  done();
}
